...
export async function xmSongLoader(dataBlob: Blob) {
	...
	const patternOrderTableBytes = await getBytes(80, 256);
	const patternOrderTable = Array.from(patternOrderTableBytes).slice(0, songLength);
	...
	for (let i = 0; i < patternCount; i++) {
		...
		const dataBytes = await getBytes(patternOffset + patternHeaderLength, packedPatternDataSize);
		const data = [];
		let byteIndex = 0;
	...
	const song = new Song({ bpm: defaultBpm, speed: defaultTempo, patternOrder: patternOrderTable, patterns });
